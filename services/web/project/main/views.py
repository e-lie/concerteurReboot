from flask import current_app, render_template, request, url_for, jsonify, redirect, send_from_directory
...
from ..models import Question, Sender, Message
...
def add_sms():
    ...
    if request.method == 'GET':
        ...
        if question:
            ...
            sender = db.session.query(Sender).filter(Sender.numHash==hashNum).first()
            if not sender:
                sender = Sender(hashNum, message)
            else:
